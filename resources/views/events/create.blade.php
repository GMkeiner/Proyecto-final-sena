    ...
    <div class="container">
        ...
        <form action="{{ isset($event) ? route('events.update', $event) : route('events.store') }}" method="POST">
            ...
            <div class="row row-cols-3 m-1">
                <button type="button" id="add_date" class="btn btn-secondary col-2">Agregar Fecha</button>
                <a href="{{ route('events.index') }}" class="btn btn-secondary col-2 ms-2">Regresar</a>
            </div>
            <button type="submit" class="btn btn-primary mt-2 ms-1 col-2">{{ isset($event) ? 'Actualizar' : 'Crear' }}</button>
        </form>
    </div>
@endsection
